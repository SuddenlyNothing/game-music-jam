...
func play_audio(name):
	...
	if audio_data[name].enable:
		if (audio_data[name].allow_interrupt and node.get_playback_position() > 0.1) or not node.is_playing():
			node.play()
...
func _process(delta):
	...
	if not is_state(state.TYPING) and $FX/Audio/Typing.playing:
		$FX/Audio/Typing.stop()
...
func _input(event: InputEvent) -> void:
	if not Engine.is_editor_hint() and event.is_action_pressed(Dialogic.get_action_button(), false, true) and autoPlayMode:
		autoPlayMode = false
		return
	
	if not Engine.is_editor_hint() and event.is_action_pressed(Dialogic.get_action_button(), false, true) and (!noSkipMode and !autoPlayMode):
